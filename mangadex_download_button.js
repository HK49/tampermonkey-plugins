...
async function download(id) {
    ...
    // only then can we define pages, because idb onsuccess event will fire even when creating new db
    ...
    document.body.appendChild(
        Object.assign(document.createElement('iframe'), {
            height: 0,
            name: `frame${id}`,
            onload: _ => process(pages, chapter).then(_ => generateZip(chapter)).catch(e => progress.error(e, id)), // start download process
...
function pagesToDownload(chapter, previousDownload) {
    return new Promise((resolve, reject) => {
        if (previousDownload) {
            let toDownload = [];
            ...
            cursorOnSaved(false, () => toDownload.push(event.target.result.value.page), () => resolve(toDownload));
        } else {
...
// throttled image load, convert buffer to blob, save it to IDB and only when all ready - zip em all
...
const resolver = () => { // get response from worker and resolve awaiting promise from Promise.all array of process function
    ...
    worker.onerror = e => console.error(`Error from worker: ${e.message} on line ${e.lineno}.`);
}
resolver();


function save([ page, buffer ], chapter) { // convert image into data in separate thread and give way to loading
